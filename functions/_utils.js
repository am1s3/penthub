...
export function containsProhibited(text) {
  ...
  const patterns = [
    /глушилк/i, /jammer/i, /деаутентифик/i, /deauth/i, /брутфорс/i, /brute\s?force/i,
    /подбор парол/i, /credential stuffing/i, /скиммер/i, /skimmer/i, /клонировани[ея] карт/i,
    /card cloning/i, /malware/i, /ransomware/i, /ботнет/i, /botnet/i, /ddos/i,
    /steal\s+password/i, /взлом аккаунта/i, /угон сессии/i, /session hijack/i
  ];
...
export async function readJson(request, maxBytes = 16384) {
  ...
  if (contentLength && Number(contentLength) > maxBytes) throw new Error('Body too large');
...
export async function createSession(env, userId, ip) {
  ...
  ).bind(tokenHash, userId, now, expiresAt, ip).run();
...
export async function getSessionUser(env, request) {
  ...
  const row = await env.DB.prepare(
    `SELECT s.token_hash AS session_token_hash, u.id AS user_id, u.username, u.is_admin, u.is_moderator, u.banned
  ...
  if (row.banned) {
    await env.DB.prepare('DELETE FROM sessions WHERE token_hash = ?').bind(row.session_token_hash).run();
    return null;
  }
...
export async function verifyTurnstile(env, request, token) {
  ...
  if (!token || typeof token !== 'string' || token.length < 8 || token.length > 4096) return false;
...
export async function getUserMetaMap(env, ids) {
  const map = {};
  ...
  const badges = await env.DB.prepare(
    `SELECT ub.user_id, b.label, b.color, b.icon
     FROM user_badges ub JOIN badges b ON b.id = ub.badge_id
     WHERE ub.user_id IN (${placeholders})`
  ).bind(...unique).all();

  const badgeMap = {};
  for (const r of badges.results || []) {
    (badgeMap[r.user_id] ||= []).push({ label: r.label, color: r.color, icon: r.icon });
  }
...
export async function rateLimit(env, rawKey, limit, windowSec) {
  ...
  try {
    ...
    if (!row) {
      await env.DB.prepare('INSERT INTO rate_limits (key, window_start, count) VALUES (?, ?, 1)').bind(key, windowStart).run();
      return true;
    }

    if (row.window_start !== windowStart) {
      await env.DB.prepare('UPDATE rate_limits SET window_start = ?, count = 1 WHERE key = ?').bind(windowStart, key).run();
      return true;
    }
...
export async function audit(env, { userId = null, action, request = null, details = '' }) {
  try {
    ...
    ).bind(userId, action, ip, safeDetails, Date.now()).run();
  } catch {
