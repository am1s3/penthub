  ...
  try {
    if (withUsername) {
      ...
      await env.DB.prepare(
        ...
        .bind(Date.now(), user.id, other.id)
        .run();

      const result = await env.DB.prepare(
        `
          SELECT pm.id, pm.body, pm.created_at, pm.read_at,
        ...
        .bind(user.id, other.id, other.id, user.id, PER_PAGE, offset)
        .all();
...
async function handlePost(request, env) {
  ...
  try {
    ...
    const inserted = await env.DB.prepare(
      'INSERT INTO private_messages (sender_id, recipient_id, body, created_at, read_at) VALUES (?, ?, ?, ?, NULL) RETURNING id'
    )
      .bind(user.id, recipient.id, messageBody, Date.now())
      .first();
