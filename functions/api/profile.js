import { json, checkOrigin, getSessionUser, rateLimit, audit, isStaff } from '../_utils.js';
...
async function getProfile(request, env) {
  ...
  try {
    const user = await env.DB.prepare(
      `SELECT u.id, u.username, u.display_name, u.created_at, u.bio, u.avatar, u.banner,
              u.is_admin, u.is_moderator, u.banned, u.profile_hidden,
    ...
    const canSeeHidden = isSelf || (viewer && isStaff(viewer));

    if (user.profile_hidden && !canSeeHidden) {
    ...
    const [threads, posts, mentions, badges] = await Promise.all([
      env.DB.prepare(
        `SELECT t.id, t.title, t.updated_at, c.name AS category_name
         ...
         WHERE t.user_id = ? AND t.deleted = 0 ORDER BY t.updated_at DESC LIMIT 5`
      ).bind(user.id).all(),
      env.DB.prepare(
        `SELECT p.id, p.body, p.created_at, t.id AS thread_id, t.title AS thread_title
         FROM posts p JOIN threads t ON t.id = p.thread_id
         WHERE p.user_id = ? AND p.deleted = 0 AND t.deleted = 0 ORDER BY p.created_at DESC LIMIT 5`
      ).bind(user.id).all(),
      env.DB.prepare(
        `SELECT m.created_at, p.id AS post_id, p.body, t.id AS thread_id, t.title AS thread_title, u.username AS author
         ...
         WHERE m.user_id = ? AND p.deleted = 0 AND t.deleted = 0 ORDER BY m.created_at DESC LIMIT 5`
      ).bind(user.id).all(),
      env.DB.prepare(
