import {
  ...
  containsProhibited, audit, isStaff, getUserMetaMap
} from '../_utils.js';
...
export function extractMentions(text) {
  ...
  let m;
  while ((m = re.exec(text))) set.add(m[2]);
  return [...set];
}
...
export async function storeMentions(env, postId, text, authorId, threadId = null) {
  ...
  const rows = await env.DB.prepare(`SELECT id FROM users WHERE username IN (${placeholders})`).bind(...names).all();
  const now = Date.now();
...
async function getPosts(request, env) {
  ...
  try {
    ...
    if (!thread || thread.deleted) return json({ error: 'Thread not found' }, 404);
    ...
    if (ids.length) {
      ...
      if (viewer) {
        ...
        for (const row of mine.results || []) myMap[row.post_id] = row.emoji;
      }
    }
...
async function createPost(request, env) {
  ...
  try {
    ...
    if (!thread || thread.deleted) return json({ error: 'Thread not found' }, 404);
    if (thread.is_locked && !isStaff(user)) return json({ error: 'Thread is locked' }, 403);
...
async function editPost(request, env) {
  ...
  if (!newBody) return json({ error: 'Text cannot be empty' }, 400);
